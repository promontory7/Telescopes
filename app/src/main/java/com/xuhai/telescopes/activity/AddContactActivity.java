...
public class AddContactActivity extends BaseActivity {
    ...
    public void searchContact(View v) {
        ...
        if (getString(R.string.button_search).equals(saveText)) {
            ...
            String str2 = getResources().getString(R.string.Is_sending_a_request);
            progressDialog.setMessage(str2);
            progressDialog.setCanceledOnTouchOutside(false);
            progressDialog.show();

            HttpUtil.getInstance().getOneUserInformation(toAddUsername, new BaseJsonHttpResponseHandle() {
                ...
                public void onSuccess(int statusCode, Header[] headers, JSONObject response) {
                    ...
                    if (statusCode == 200) {
                        try {
                            ...
                            if (userjson == null) {
                                runOnUiThread(new Runnable() {
                                    public void run() {
                                        ...
                                        Toast.makeText(getApplicationContext(), R.string.can_not_found_user, Toast.LENGTH_LONG).show();
                                    }
                                });
                                return;
                            }
                            ...
                            progressDialog.dismiss();
                            Log.e(toAddUsername + "用户信息", userjson.toString());
                            toAddUser = setUserFromJson(userjson);

                            Log.e("findUser,toAddUser", toAddUser.toString());
                            //服务器存在此用户，显示此用户和添加按钮
                ...
                public void onFailure(final int statusCode, Header[] headers, Throwable throwable, final JSONObject errorResponse) {
                    ...
                    runOnUiThread(new Runnable() {
                        public void run() {
                            progressDialog.dismiss();
                            Log.e("getOneUserFromSever", "查找用户失败" + statusCode + errorResponse.toString());

                            Toast.makeText(getApplicationContext(), "查找用户失败" + statusCode, Toast.LENGTH_LONG).show();
                        }
                    });
                }
            });
    ...
    public void addContact(View view) {
        ...
        Log.e("toAddUser", toAddUser.toString());
        HttpUtil.getInstance().addFriends(Integer.parseInt(toAddUser.getUserId()), new BaseJsonHttpResponseHandle() {

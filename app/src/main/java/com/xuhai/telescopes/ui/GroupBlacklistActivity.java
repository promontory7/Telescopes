...
public class GroupBlacklistActivity extends BaseActivity {
	...
	protected void onCreate(Bundle arg0) {
		super.onCreate(arg0);
		setContentView(com.xuhai.telescopes.R.layout.em_activity_group_blacklist);

		progressBar = (ProgressBar) findViewById(com.xuhai.telescopes.R.id.progressBar);
		listView = (ListView) findViewById(com.xuhai.telescopes.R.id.list);

		groupId = getIntent().getStringExtra("groupId");
		// 注册上下文菜单
		registerForContextMenu(listView);
		final String st1 = getResources().getString(com.xuhai.telescopes.R.string.get_failed_please_check);
	...
	public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
		...
		getMenuInflater().inflate(com.xuhai.telescopes.R.menu.em_remove_from_blacklist, menu);
	}

	@Override
	public boolean onContextItemSelected(MenuItem item) {
		if (item.getItemId() == com.xuhai.telescopes.R.id.remove) {
	...
	void removeOutBlacklist(final String tobeRemoveUser) {
		final String st2 = getResources().getString(com.xuhai.telescopes.R.string.Removed_from_the_failure);
		try {
			// 移出黑民单
	...
	private class BlacklistAdapter extends ArrayAdapter<String> {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			if (convertView == null) {
				convertView = View.inflate(getContext(), com.xuhai.telescopes.R.layout.ease_row_contact, null);
			}

			TextView name = (TextView) convertView.findViewById(com.xuhai.telescopes.R.id.name);

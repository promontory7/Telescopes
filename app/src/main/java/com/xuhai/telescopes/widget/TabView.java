...
public class TabView extends View {
    ...
    private float  divider = 12;
    private final float SCALE_RATE = 0.8f;
    private float maxLength;
    private MoveAnimation moveForwardAnimation;
    private MoveBackAnimation moveBackAnimation;
    ...
    public void setTabPosition(int  position){
        ...
        if(position== 0){
            startAnimation(moveBackAnimation);
        }else if(position == 1){
            startAnimation(moveForwardAnimation);
        }
    }
    ...
    private void init() {
        ...
        paintText.setTextSize(getContext().getResources().getDimensionPixelOffset(R.dimen.tab_radius));
        radius = getContext().getResources().getDimensionPixelOffset(R.dimen.tab_radius);
        divider = getContext().getResources().getDimensionPixelOffset(R.dimen.tab_radius);
        circle = new Circle();
        circle.center = new float[]{(radius*2 + divider), radius * (1f + SCALE_RATE)};
        circle.radius = radius / 4*3;

        circleLeft = new Circle();
        circleLeft.center = new float[]{(radius * 2 + divider), radius * (1f + SCALE_RATE)};
        circleLeft.radius = radius;

        circleRight = new Circle();
        circleRight.center = new float[]{(radius * 2 + divider)*2, radius * (1f + SCALE_RATE)};
        circleRight.radius = radius;

        maxLength = 2*radius+divider;
        moveForwardAnimation = new MoveAnimation();
        moveForwardAnimation.setDuration(1000);
        moveForwardAnimation.setInterpolator(new AccelerateDecelerateInterpolator());

        moveBackAnimation = new MoveBackAnimation();
        moveBackAnimation.setDuration(1000);
        moveBackAnimation.setInterpolator(new AccelerateDecelerateInterpolator());
    }
    ...
    private class MoveAnimation extends Animation {
        ...
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            ...
            circle.center[0] = maxLength * interpolatedTime+radius * 2 + divider;
            invalidate();
        }
    }

    private class MoveBackAnimation extends Animation {

        @Override
        protected void applyTransformation(float interpolatedTime, Transformation t) {
            super.applyTransformation(interpolatedTime, t);
            circle.center[0] = maxLength*((float)1 - interpolatedTime)+radius * 2 + divider;
            invalidate();
        }
    }
     ...
     * @param maxDistance
     */
    private void metaball(Canvas canvas, Circle circle2, Circle circle1, float v, float handle_len_rate, float maxDistance) {
...
//        Log.d("Metaball", "sp1:" + Arrays.toString(sp1) + ",sp2:" + Arrays.toString(sp2) + ",sp3:" + Arrays.toString(sp3) + ",sp4:" + Arrays.toString(sp4));
    ...
    protected void onDraw(Canvas canvas) {
        ...
        metaball(canvas, circleLeft, circle, 0.6f, handle_len_rate, radius * 2f);
        metaball(canvas, circleRight, circle, 0.6f, handle_len_rate, radius * 2f);
//        canvas.drawText("�ں�", circleLeft.center[0], circleLeft.center[1], paintText);
//        canvas.drawText("����", circleRight.center[0], circleRight.center[1], paintText);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                if(position == 0){
                        if(event.getX()>=(circleRight.center[0]-circleRight.radius)
                            &&event.getX()<=(circleRight.center[0]+circleRight.radius)
                            &&event.getY()>=(circleRight.center[1]-circleRight.radius)
                            &&event.getY()<=(circleRight.center[1]+circleRight.radius)){
                        startAnimation(moveForwardAnimation);
                            position = 1;
                            if(tabListener != null){
                                tabListener.changeTab(position);
                            }

                    }
                }else{
                    if(event.getX()>=(circleLeft.center[0]-circleRight.radius)
                            &&event.getX()<=(circleLeft.center[0]+circleRight.radius)
                            &&event.getY()>=(circleLeft.center[1]-circleRight.radius)
                            &&event.getY()<=(circleLeft.center[1]+circleRight.radius)){
                        startAnimation(moveBackAnimation);
                        position = 0;
                        if(tabListener != null){
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        setMeasuredDimension(resolveSizeAndState((int) ((3) * (radius * 2 + divider)), widthMeasureSpec, 0),
                resolveSizeAndState((int) (2 * radius * (1+SCALE_RATE)), heightMeasureSpec, 0));
    }
